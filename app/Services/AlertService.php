...
class AlertService
{
    public function checkBudgetThresholds(BudgetCategory $budgetCategory): void
    {
        ...
        $alertType = match (true) {
            ...
            $percentage >= $budgetCategory->alert_threshold => 'threshold_'.$budgetCategory->alert_threshold,
        ...
        if ($percentage >= 50 && $percentage < 100) {
            ...
            if (! $alreadyExists) {
                $this->createGoalAlert($goal, 'goal_progress', 'info', [
                    ...
                    'message_ar' => "وصلت إلى {$percentage}% من هدف \"{$goal->title}\"، أكمل!",
                    'message_en' => "You reached {$percentage}% of \"{$goal->title}\". Keep it up!",
                ]);
            }
        }
    }

    protected function createGoalAlert(SavingGoal $goal, string $type, string $severity, array $messages): void
    {
        Alert::create([
